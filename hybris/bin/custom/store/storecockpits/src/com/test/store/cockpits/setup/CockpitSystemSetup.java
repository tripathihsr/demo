 ...
 */
package com.test.store.cockpits.setup;
...
import com.test.store.cockpits.constants.StoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = StoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(StoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(StoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						StoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/storecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
